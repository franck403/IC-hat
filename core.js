        ...
        if (data2.val().name != null) {
            if (data2.val().message != null || data2.val().message != undefined) {
                if (data2.val().type == "message" && data2.val().mtype != 'embed') {
                    ...
                    if (data2.val().email == myEmail) {
                        var DateNow = data2.val().date
                        var html = `<div class="bubble me ${class_added}"><img src="${url}" class="messageProfileMe"><audio-player class="type-img" data-state="load" data-date="${DateNow}" src="${data2.val().message}"></audio-player>${tooltip}</div>`
                    } else {
                        var DateNow = data2.val().date
                        var html = `<div class="bubble you ${class_added}"><img src="${url}" class="messageProfile"><div class="bubble-name">${data2.val().name}</div><div><audio-player class="type-img " data-date="${DateNow}" data-state="load" src="${data2.val().message}"></audio-player></div>${tooltip}</div>`
                    }
                } else { }
                try {
                    return [d1, html]
